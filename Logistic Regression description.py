...
def one_hot_encoder(target):
    ...
    """
    target_name, y = np.unique(target, return_inverse=True) #use np.unique to create an array of only 1-dimensional arrays.
    one_hot = np.zeros((y.size, y.max()+1)) #Create a zero array with np.zeros next Row and main are defined.
    one_hot[np.arange(y.size),y] = 1 #Assign an arange = 1
...
def find_gradient(X, h, y):
    ...
    """
    gradient = np.dot(X.T,(h - y))/y.shape[0] #Multiplying a matrix using np.dot
    return gradient
...
def find_accuracy(y, preds):
    ...
    Outpu:  accuracy
    """
    pred_labels = [] #Create a list to store values. This list is called pred_labels
    for row in y:
        pred_labels.append(np.argmax(row))# appendคือการเพิ่มค่าที่สูงที่สุดโดยใช้ np.argmaxในการหาค่าที่สูงที่สุดในแถวเข้าไปในlist
        
    num_correct = 0 #กำหนดการวนซ้ำ
    for i in range(len(pred_labels)):# Finding the number in list name pred_labels
        if pred_labels[i] == preds[i]: #set conditions
            num_correct += 1 #num_crrect = num_crrect + 1
    acc = num_correct / len(pred_labels)   #Accuracy calculation formula
